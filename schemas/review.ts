...
export default defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      title: 'Created By',
      type: 'object',
      description: 'Owner of the review session',
      fields: [
        ...
        { name: 'reviewUrl', type: 'url', title: 'Owner Review URL', description: 'Full URL for the owner to access and reply to the review', readOnly: true },
      ],
      validation: (Rule) => Rule.required(),
    }),
    defineField({
      ...
      of: [
        {
          ...
          fields: [
            { name: 'name', type: 'string', title: 'Name', validation: (Rule) => Rule.required() },
            { name: 'email', type: 'string', title: 'Email', validation: (Rule) => Rule.required().email() },
            { name: 'reviewUrl', type: 'url', title: 'Review URL', description: 'Full URL for this recipient to access the review', readOnly: true },
    ...
    defineField({
      name: 'isOwnerAccess',
      title: 'Is Owner Access',
      type: 'boolean',
      description: 'Flag to identify if this is an owner access token (for private mode)',
      initialValue: false,
      hidden: true,
    }),
  ],
